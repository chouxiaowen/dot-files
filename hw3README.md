...
This step takes 5-15 minutes. If the build is successful, you will see the following line: 
...
Everytime you modify the source or reconfigure the server, you will have to restart the server for your changes to take effect. You can stop the server using the following command:

    $ $HOME/pgsql/bin/pg_ctl -D ~/pgsql/data stop

### The PSQL Client Application

When the server is up and running, we can use the psql client application to connect to the server and pose a few SQL queries. To start, open a new terminal session (if the original terminal session is running the server), and start psql:
...
Note that every SQL query must be terminated with a semi-colon (;). Next, we insert a few rows into the table, and then run a simple SELECT query:
...
Of these files, `freelist.c` is the primary file you will need to modify (although modifications to other files may also be necessary). To quickly browse the PostgreSQL source code, you may find tools like cscope and tags to be helpful.
...
Stop the PostgreSQL server if you already started it, and use the following command to re-start the server:
    ...
    $HOME/pgsql/bin/pg_ctl start -D $HOME/pgsql/data -l lru64.log -o "-B 64 -N 1 -o '-te -fm -fh' --buffer-replacement-policy=lru --autovacuum=off"
...
Note: you must run these queries in order. These queries have been 


since you start the server. Don't restart the server.
...
In total, your file will have 16 lines.
...
You will need to submit the following 4 files.

#### performance.txt

Report the hit rates in 16 lines. Please refer to the [Reporting Test Result](https://github.com/radlab/cs186-fa12/blob/master/hw3/README.md#reporting-test-results) section for details.

#### hw3.diff

This diff file describes the changes you made to the PostgreSQL source code. You can generate this file as follows:

    cd postgresql-8.4.2
    git diff origin/master -- . > ~/hw3.diff

Make sure the generated diff file includes all the changes you have made to the code. 
Make sure it doesn't include new files that are not provided by us, e.g., new source files or the compiler-generated files. 
To avoid including additional files, don't ``git add'' any files to the git index. To list only the file names in your diff file, use:

    git diff --name-only origin/master -- . > ~/hw3.diff

We will be testing and grading your code generated by patching this diff file to postgres source. We strongly recommend you to review the diff file manually.

#### README

If you are using slip days, your README should contain a single digit indicating the number of slip days you wish to use. For example, if you wanted to use two slip days, the README would consist of only one line with the number: `2`. If you are not using any slip days, the README should be empty.

#### MY.PARTNERS

This is auto-generated for you when you run the submit hw3 script.


You can transfer the files to the inst machines with 
...
On an inst machine, run `submit hw3`:
